...
const setupMaster = (httpServer, opts) => {
  ...
  const computeWorkerId = (data) => {
    ...
    if (match) {
      ...
      if (sid && !workerId) {
        console.warn(`No worker id for request with sid ${sid}`);
      }
      if (sid && workerId && !cluster.workers[workerId]) {
        console.warn(`No worker with id ${workerId} in cluster for request with sid ${sid}`);
      }
  ...
  httpServer.on("connection", (socket) => {
    ...
    socket.on("data", (buffer) => {
      ...
      const requestLine = data.substring(0, data.indexOf("\r\n"));
      workerId = computeWorkerId(requestLine);

      const head = data.substring(0, data.indexOf("\r\n\r\n")).toLowerCase();
      const headersArray = head.split("\r\n").slice(1).map(Function.prototype.call.bind(String.prototype.trim));
      let mayHaveMultipleChunks = false;
      for (const header of  headersArray) {
        if(header.startsWith("transfer-encoding:") && header.includes("chunked")) { // https://datatracker.ietf.org/doc/html/rfc7230#section-3.3.1
          mayHaveMultipleChunks = true;
          break;
        }
      }
